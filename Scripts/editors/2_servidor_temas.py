# -*- coding: utf-8 -*-
"""
Servidor Web para Editor de Temas com edição direta dos arquivos JSON
"""
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Editor de Temas - ResumosDireito</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <script>
        tailwind.config = {
            theme: {
...
<body class="bg-gray-50">
    <div class="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1 bg-white rounded-lg shadow-md p-6">
                ...
                <div id="lista-temas" class="space-y-2 max-h-[600px] overflow-y-auto">
            ...
            <div class="lg:col-span-2 bg-white rounded-lg shadow-md p-6">
                ...
                <form id="formulario" class="space-y-4 hidden">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Tese *</label>
    ...
    <script>
        ...
        let temas = [];
        let todastemas = {}; // Armazena todas as Temas de todos os tribunais
        let categorias = {};
        let temasSelecionadas = new Set();
        let textoPesquisa = '';
        ...
        async function carregartemas(tribunal) {
            try {
                if (tribunal === 'todos') {
                    // Carregar todos os tribunais
                    const [stfRes, stjRes] = await Promise.all([
                        fetch('/api/temas/stf'),
                        fetch('/api/temas/stj')
                    ]);
                    
                    const [stfData, stjData] = await Promise.all([
                        stfRes.json(),
                        stjRes.json()
                    ]);
                    ...
                    temas = [
                        ...todastemas.stf.map(s => ({...s, _tribunal: 'STF'})),
                        ...todastemas.stj.map(s => ({...s, _tribunal: 'STJ'}))
                    ].sort((a, b) => a.numero - b.numero);
                    
                } else {
                    const response = await fetch(`/api/temas/${tribunal}`);
                    const data = await response.json();
                    todastemas[tribunal] = data.temas || [];
                    temas = todastemas[tribunal].map(s => ({...s, _tribunal: tribunal.toUpperCase()}));
                }
                
                atualizarLista();
                atualizarContador();
                criarFiltroCores(); // Atualizar filtro de cores após carregar Temas
            } catch (error) {
                mostrarToast('Erro ao carregar Temas', 'error');
            }
        }
        ...
        function atualizarLista() {
            ...
            // Filtrar Temas pela pesquisa e por cor
            let temasFiltradas = temas;
            
            // Filtro de texto
            if (textoPesquisa) {
                const termo = textoPesquisa.toLowerCase();
                temasFiltradas = temasFiltradas.filter(s => 
                    s.tese.toLowerCase().includes(termo) ||
            ...
            if (coresFiltradas.size > 0) {
                temasFiltradas = temasFiltradas.filter(s => coresFiltradas.has(s.cor));
            }
            
            // Atualizar contador de resultados
            const resultadoDiv = document.getElementById('resultado-pesquisa');
            if (textoPesquisa || coresFiltradas.size > 0) {
                resultadoDiv.textContent = `${temasFiltradas.length} de ${temas.length} Temas`;
            } else {
                resultadoDiv.textContent = '';
            }
            
            if (temasFiltradas.length === 0) {
                lista.innerHTML = '<p class="text-gray-500 text-center py-8">Nenhuma Tema encontrada</p>';
                return;
            }

            lista.innerHTML = temasFiltradas.map((s, i) => {
                // Encontrar índice original
                const indiceOriginal = temas.indexOf(s);
                const selecionada = temasSelecionadas.has(indiceOriginal);
                ...
                <div class="sumula-item p-3 border ${borderClass} rounded-lg cursor-pointer hover:bg-gray-50 hover:border-blue-400 relative">
                    <div class="flex items-start gap-2">
                        ...
                        <div onclick="editarSumula(${indiceOriginal})" class="flex-1">
                            ...
                            ${textoPesquisa ? `<p class="text-xs text-gray-600 mt-1 line-clamp-2">${s.tese}</p>` : ''}
                        </div>
                    </div>
                </div>
            `}).join('');
        ...
        function toggleSelecionada(indice) {
            if (temasSelecionadas.has(indice)) {
                temasSelecionadas.delete(indice);
            } else {
                temasSelecionadas.add(indice);
            }
            atualizarLista();
        }

        function desmarcarTodas() {
            temasSelecionadas.clear();
            atualizarLista();
        }
        ...
        function atualizarAcoesMassa() {
            ...
            count.textContent = temasSelecionadas.size;
            
            if (temasSelecionadas.size > 0) {
        ...
        async function aplicarCorEmMassa() {
            ...
            if (temasSelecionadas.size === 0) {
                mostrarToast('Selecione pelo menos uma Tema', 'error');
                return;
            }
            
            const quantidade = temasSelecionadas.size;
            
            if (!confirm(`Alterar cor de ${quantidade} Tema(s)?`)) {
                return;
            }
            
            // Aplicar cor nas Temas selecionadas
            temasSelecionadas.forEach(indice => {
                temas[indice].cor = corSelecionada;
            });
            
            // Salvar - precisa separar por tribunal no modo TODOS
            if (tribunalAtual === 'todos') {
                // Reorganizar Temas por tribunal
                
                const porTribunal = { stf: [], stj: [] };
        ...
        async function salvarTribunal(tribunal, temasArray) {
            const data = {
                tribunal: tribunal.toUpperCase(),
                total: temasArray.length,
                temas: temasArray
            };
        ...
        document.addEventListener('DOMContentLoaded', () => {
            const inputPesquisa = document.getElementById('pesquisa-temas');
            if (inputPesquisa) {
        ...
        function criarFiltroCores() {
            ...
            // Contar quantas Temas de cada cor existem
            ...
            const coresComtemas = coresConfig.filter(c => coresUsadas.has(c.cor));
        ...
        async function atualizarContador() {
            for (const tribunal of ['stf', 'stj']) {
                const response = await fetch(`/api/temas/${tribunal}`);
                const data = await response.json();
                document.getElementById(`count-${tribunal}`).textContent = (data.temas || []).length;
            }
            ...
            document.getElementById('count-todos').textContent = totalStf + totalStj;
        }
        ...
        function novaSumula() {
            if (tribunalAtual === 'todos') {
                mostrarToast('Selecione um tribunal específico para adicionar Tema', 'error');
                return;
            }
        ...
        async function salvarSumula(sumula) {
            try {
                if (tribunalAtual === 'todos') {
                    ...
                    temas.forEach(s => {
                        if (!s._tribunal) {
                            console.error('Tema sem tribunal:', s);
                            return;
                        }
                        const trib = s._tribunal.toLowerCase();
                        const { _tribunal, ...sumulaLimpa } = s;
                    ...
                    const data = {
                        ...
                        total: temasLimpas.length,
                        temas: temasLimpas
                    };
                    
                    const response = await fetch(`/api/temas/${tribunalAtual}`, {
                        method: 'POST',
                        headers: {'Content-Type': 'application/json'},
                        body: JSON.stringify(data)
                    });
        ...
        document.getElementById('formulario').addEventListener('submit', async (e) => {
            ...
            const tema = {
                ...
                tese: document.getElementById('texto').value,
            ...
            if (indice === '') {
                ...
                const tribunalOriginal = temas[indiceInt]._tribunal;
                temas[indiceInt] = tema;
                if (tribunalOriginal) {
                    temas[indiceInt]._tribunal = tribunalOriginal;
                }
            }
        ...
        async function excluirSumula() {
            const indice = document.getElementById('indice-edicao').value;
            if (indice !== '' && confirm('Tem certeza que deseja excluir esta Tema?')) {
                temas.splice(parseInt(indice), 1);
                await salvarSumula();
            }
        }
        ...
        async function gerarHTML() {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    setTimeout(() => {
                        alert('HTML atualizado!\\n\\nVerifique o arquivo penal-public/public\temas.html');
                    }, 500);
                } else {
...
if __name__ == '__main__':
    ...
    print("\n📝 Funcionalidades:")
    print("   • Editar Temas diretamente nos arquivos JSON")
    print("   • Adicionar e excluir Temas")
